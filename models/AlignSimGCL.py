...
class AlignSimGCL(TrainableModelBase):
    @dataclass
    ...
    def __init__(
        ...
        super(AlignSimGCL, self).__init__(model_config=model_config, loss_config=loss_config)
    ...
    def _get_initial_embeddings(self) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def _get_propagated_embeddings(self, perturbed: bool = False) -> tuple[torch.Tensor, torch.Tensor]:
        user_embedding, item_embedding = self._get_initial_embeddings()
    ...
    def get_loss(
        ...
        reg_loss = self.model_config.reg_weight * 0.5 * sum(
            parameter.pow(2).sum() for parameter in self.adapter.parameters()
        )
